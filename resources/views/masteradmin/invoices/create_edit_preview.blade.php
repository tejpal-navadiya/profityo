...
<section class="content px-10">
    <div class="container-fluid">
        ...
        <div class="card card-default">
            ...
            <?php //dd($previewData); ?>
            <?php if (isset($previewData['invoices_id'])): ?>
              <form id="items-form1"
                  action="{{ route('business.invoices.update', ['invoices_id' => $previewData['invoices_id']]) }}"
                  method="POST">
                  @csrf
                  @method('Patch')
              <?php else: ?>
                <form id="items-form1" action="{{ route('business.invoices.store') }}" method="POST">
                  @csrf
              <?php endif; ?>
                    ...
                    <div class="col-lg-9 col-md-12">
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="sale_estim_number" id="invoicenumber"
                                        placeholder="" value="{{ session('previewData.sale_estim_number', $newId) }}">
                            ...
                            <div class="form-group">
                                ...
                                <input type="hidden" id="from-datepicker-hidden" value="{{ session('previewData.sale_estim_date') }}" />
                      ...
                      <div class="input-group-append" data-target="#estimatevaliddate" data-toggle="datetimepicker">
                        <div class="input-group-text"><i class="fa fa-calendar-alt"></i></div>
                                  ...
                                  <input type="hidden" id="to-datepicker-hidden" value="{{ session('previewData.sale_estim_valid_date') }}" />

                                        <x-flatpickr id="to-datepicker" value="{{ session('previewData.sale_estim_valid_date', old('sale_estim_valid_date')) }}"
                                    ...
                                    <span class="error-message" id="error_sale_estim_valid_date"
                                        style="color: red;"></span>
                                    <!-- <p class="within_day">Within 7 days</p> -->
                                    <p class="within_day">Within <span id="total-days">{{ session('previewData.sale_total_days') ?? '0' }}</span> days</p>
                                    <input type="hidden" id="hidden-total-days" name="sale_total_days" value="{{ session('previewData.sale_total_days') ?? '0' }}">
  ...
  <div class="modal fade" id="editcolum" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form method="post" id="estimateForm" action="{{ route('invoicesmenus.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                @csrf
                @method('patch')
                <div class="modal-body">
...
<script>
    ...
    function submitFormViaAjax(formData) {
      ...
      var method;
    <?php if (isset($previewData['invoices_id'])): ?>
        url = "{{ route('business.invoices.update', ['invoices_id' => $previewData['invoices_id']]) }}";
        method = 'PATCH';
    <?php else: ?>
