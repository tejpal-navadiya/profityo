...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2 align-items-center justify-content-between">
            <div class="col-auto">
                <h1 class="m-0">{{ __('Edit Recurring Invoice') }}</h1>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('business.home') }}">Dashboard</a></li>
                    <li class="breadcrumb-item active">{{ __('Edit Recurring Invoice') }}</li>
                </ol>
            </div><!-- /.col -->
            <div class="col-auto">
    ...
    <div class="container-fluid">
        ...
        <div class="card card-default">
            ...
            <?php if (isset($previewData['reinvoices_id'])): ?>
              <form id="items-form1"
                  action="{{ route('business.recurring_invoices.update', ['reinvoices_id' => $previewData['reinvoices_id']]) }}"
                  method="POST">
                  @csrf
                  @method('Patch')
              <?php else: ?>
                <form id="items-form1" action="{{ route('business.recurring_invoices.store') }}" method="POST">
                  @csrf
              <?php endif; ?>
                    ...
                    <div class="row justify-content-between">
                        <div class="col-md-3 px-10">
                            <div class="business_logo_uplod_box">
                                        ...
                                        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                <div class="modal-body pad-1 text-center">
                                                    ...
                                                    <p class="company_details_text">Removing your logo will remove it from
                                                        all existing and future
                                                        invoices and invoice. Are you sure you want to remove your
                                                        business logo?</p>
                                ...
                                @else
                                    <form method="post" id="editBusinessImageForm" class="mt-6 space-y-6"
                                        ...
                                        <p class="upload_text">Browse or Drop your Logo Here Maximum 5MB in Size. JPG, PNG,
                                            or GIF Formats.
                        ...
                        <div class="col-md-7 px-10">
                            <div class="row justify-content-end">
                                <div class="col-md-7 float-sm-right">
                                    <input type="text"
                                        class="form-control text-right @error('sale_estim_title') is-invalid @enderror"
                                        name="sale_estim_title" id="invoicetitle" value="{{ session('previewData')['sale_estim_title'] ?? old('sale_estim_title') }}"
                            ...
                            <div class="row justify-content-end">
                                <div class="col-md-7 float-sm-right px-10">
                                    ...
                                    <input type="text"
                                        class="form-control text-right @error('sale_estim_summary') is-invalid @enderror"
                                        name="sale_estim_summary" id="invoicesummary"
                                        value="{{ session('previewData')['sale_estim_summary'] ?? old('sale_estim_summary') }}"

                                        placeholder="Summary (e.g. project name, description of invoiice)">
                            ...
                            <div class="px-10">
                                <p class="company_business_name text-right">
                                    {{ $businessDetails->bus_company_name ?? '' }}</p>
                                <p class="company_details_text text-right">{{  $businessDetails->bus_address1 ?? '' }}
                                </p>
                                <p class="company_details_text text-right">{{  $businessDetails->bus_address2 ?? '' }}
                                </p>
                                <p class="company_details_text text-right">{{ $businessDetails->state->name ?? '' }},
                                    {{  $businessDetails->city_name ?? '' }} {{ $businessDetails->zipcode ?? '' }}
                                </p>
                                <p class="company_details_text text-right">{{  $businessDetails->country->name ?? '' }}
                                </p>
                                <p class="company_details_text text-right">Phone:
                                    {{  $businessDetails->bus_phone ?? ''}}</p>
                                <p class="company_details_text text-right">Mobile:
                                    {{  $businessDetails->bus_mobile ?? ''}}</p>
                                <p class="company_details_text text-right">{{  $businessDetails->bus_website ?? '' }}
                                </p>
                            </div>
        ...
        <div class="card card-default">
            ...
            <div class="card-body2">
                <div class="row justify-content-between pad-3">
                  @if(!empty(session('previewData.sale_cus_id')))
                    ...
                    <div class="col-md-3">
                        <div id="customerInfo">
                            ...
                            <p class="company_details_text">{{ $singlecustomer->sale_cus_business_name ?? '' }}</p>
                            <p class="company_details_text">
                                {{ $singlecustomer->sale_cus_first_name ?? '' }} {{ $singlecustomer->sale_cus_last_name ?? '' }}
                            </p>
                            <p class="company_details_text">{{ $singlecustomer->sale_cus_email ?? '' }}</p>
                            <p class="company_details_text">{{ $singlecustomer->sale_cus_phone ?? '' }}</p>
                            <div class="edit_es_text" data-toggle="modal"
                                data-target="#editcustor_modal_{{ $singlecustomer->sale_cus_id ?? '' }}"
                                data-id="{{ $singlecustomer->sale_cus_id ?? '' }}">
                        ...
                        <div class="add_customer_list1" style="display: none;">
                  ...
                  @else
                      <!-- If 'sale_cus_id' does not exist in session -->
                  ...
                  @endif
                    ...
                    <div class="col-lg-9">
                        <div class="row in-number-row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="invoicecustomerref">P.O./S.O. Number</label>
                                    <input type="text" class="form-control" name="sale_estim_customer_ref"
                                        id="invoicecustomerref" value="{{ session('previewData.sale_estim_customer_ref', old('sale_estim_customer_ref')) }}" placeholder="">
                                    <span class="error-message" id="error_sale_estim_customer_ref"
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="sale_re_inv_payment_due_id" id="sale_re_inv_payment_due_id" class="form-control select2" style="width: 100%;">
                                    <option value="" {{ session('previewData.sale_re_inv_payment_due_id') == null ? 'selected' : '' }}>On Receipt</option>
                                    <option value="7" {{ session('previewData.sale_re_inv_payment_due_id') == 7 ? 'selected' : '' }}>Within 7 Days</option>
                                    <option value="14" {{ session('previewData.sale_re_inv_payment_due_id') == 14 ? 'selected' : '' }}>Within 14 Days</option>
                                    <option value="30" {{ session('previewData.sale_re_inv_payment_due_id') == 30 ? 'selected' : '' }}>Within 30 Days</option>
                                    <option value="45" {{ session('previewData.sale_re_inv_payment_due_id') == 45 ? 'selected' : '' }}>Within 45 Days</option>
                                    <option value="60" {{ session('previewData.sale_re_inv_payment_due_id') == 60 ? 'selected' : '' }}>Within 60 Days</option>
                                    <option value="90" {{ session('previewData.sale_re_inv_payment_due_id') == 90 ? 'selected' : '' }}>Within 90 Days</option>
                                </select>
                                </div>
                            </div>
                        </div>
        ...
        <tr>
            ...
            @if(isset($sessionData['Items']) && $sessionData['Items'] !== '')
                ...
                @if(isset($sessionData['Items_other']) && $sessionData['Items_other'] !== '')
                    {{ $sessionData['Items_other'] }} <!-- If Amount_other is set, show it -->
                @else
                    {{ $sessionData['Items'] }} <!-- Otherwise, show Amount -->
                @endif
                    <span class="text-danger">*</span>

                        <!-- {{ $sessionData['Items'] ?? '' }} <span class="text-danger">*</span> -->
                        @if(isset($sessionData['hide_items']) && $sessionData['hide_items'] === 'on')
            ...
            @if(isset($sessionData['Units']) && $sessionData['Units'] !== '')
                <th style="width: 15%;" id="unitsHeader" class="">
                    @if(isset($sessionData['Units_other']) && $sessionData['Units_other'] !== '')
                        {{ $sessionData['Units_other'] }} <!-- If Amount_other is set, show it -->
                    @else
                        {{ $sessionData['Units'] }} <!-- Otherwise, show Amount -->
                    @endif
                    <span class="text-danger">*</span>
                    <!-- {{ $sessionData['Units'] ?? '' }} <span class="text-danger">*</span> -->
                    @if(isset($sessionData['hide_units']) && $sessionData['hide_units'] === 'on')
                        <i class="fas fa-eye-slash" aria-hidden="true"></i>
                    @endif
                </th>
            @else
            ...
            <!-- Price Column -->
            @if(isset($sessionData['Price']) && $sessionData['Price'] !== '')
                <th style="width: 15%;" id="priceHeader" class="">
                    @if(isset($sessionData['Price_other']) && $sessionData['Price_other'] !== '')
                        {{ $sessionData['Price_other'] }} <!-- If Amount_other is set, show it -->
                    @else
                        {{ $sessionData['Price'] }} <!-- Otherwise, show Amount -->
                    @endif
                    <span class="text-danger">*</span>
                    @if(isset($sessionData['hide_price']) && $sessionData['hide_price'] === 'on')
                        <i class="fas fa-eye-slash" aria-hidden="true"></i>
                    @endif
                </th>
            @else
            ...
            <!-- Amount Column -->
            @if(isset($sessionData['Amount']) && $sessionData['Amount'] !== '')
                <th style="width: 15%;" id="amountHeader">
                    @if(isset($sessionData['Amount_other']) && $sessionData['Amount_other'] !== '')
                        {{ $sessionData['Amount_other'] }} <!-- If Amount_other is set, show it -->
                    @else
                        {{ $sessionData['Amount'] }} <!-- Otherwise, show Amount -->
                    @endif
                    <span class="text-danger">*</span>

                    @if(isset($sessionData['hide_amount']) && $sessionData['hide_amount'] === 'on')
                        <i class="fas fa-eye-slash" aria-hidden="true"></i> <!-- Icon to indicate visibility is hidden -->
                    @endif
                </th>
            @else
                <th style="width: 15%;" id="amountHeader">
            ...
            <th class="text-right">Actions</th> <!-- New column for actions -->
                        ...
                        <tbody>
                            @if(session('previewData.items')) <!-- Check if there are items in previewData -->
                                @foreach(session('previewData.items') as $index => $item)
                                    <tr class="item-row">
                                        <td>
                                            <div>
                                                <select class="form-control select2" name="items[][sale_product_id]" style="width: 100%;">
                                                    ...
                                                    @foreach($products as $product)
                                                        <option value="{{ $product->sale_product_id }}" 
                                                                {{ old('items.' . $index . '.sale_product_id', $item['sale_product_id']) == $product->sale_product_id ? 'selected' : '' }}>
                                                            {{ $product->sale_product_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                <span class="error-message" id="error_items_{{ $index }}_sale_product_id" style="color: red;"></span>

                                                <input type="text" class="form-control px-10" 
                                                    name="items[][sale_estim_item_desc]" 
                                                    placeholder="Enter item description" 
                                                    value="{{ old('items.' . $index . '.sale_estim_item_desc', $item['sale_estim_item_desc'] ?? '') }}">
                                                <span class="error-message" id="error_items_{{ $index }}_sale_estim_item_desc" style="color: red;"></span>
                                            </div>
                                        </td>
                                        <td>
                                            <input type="number" class="form-control" min="0"
                                                name="items[][sale_estim_item_qty]" 
                                                placeholder="Enter item Quantity" 
                                                value="{{ old('items.' . $index . '.sale_estim_item_qty', $item['sale_estim_item_qty'] ?? '') }}">
                                            <span class="error-message" id="error_items_{{ $index }}_sale_estim_item_qty" style="color: red;"></span>
                                        </td>
                                        <td>
                                            <input type="text" name="items[][sale_estim_item_price]" min="0"
                                                class="form-control" placeholder="Enter item Price"
                                                value="{{ old('items.' . $index . '.sale_estim_item_price', $item['sale_estim_item_price'] ?? '') }}">
                                            <span class="error-message" id="error_items_{{ $index }}_sale_estim_item_price" style="color: red;"></span>
                                        </td>

                                        <td>
                                            <select class="form-control select2" name="items[][sale_estim_item_tax]" style="width: 100%;">
                                                @foreach($salestax as $salesTax)
                                                    <option value="{{ $salesTax->tax_id }}"
                                                            {{ old('items.' . $index . '.sale_estim_item_tax', $item['sale_estim_item_tax'] ?? '') == $salesTax->tax_id ? 'selected' : '' }}
                                                            data-tax-rate="{{ $salesTax->tax_rate }}">
                                        ...
                                        <td class="item-price text-right">
                                            {{ old('items.' . $index . '.sale_estim_item_price', $item['sale_estim_item_price'] ?? '0.00') }}
                                        </td>
                                        <td class="text-right"><i class="fa fa-trash delete_icon_grid delete-item"></i></td>
                                    </tr>
                                @endforeach
                            @else
                                <!-- Template Row for New Item -->
                                <tr class="item-row" id="item-row-template">
                                    ...
                                    <td>
                                        <input type="number" class="form-control" min="0" name="items[][sale_estim_item_qty]" placeholder="Enter item Quantity">
                                        <span class="error-message" id="error_items_0_sale_estim_item_qty" style="color: red;"></span>
                                    </td>
                                    <td>
                                        <input type="text" name="items[][sale_estim_item_price]" min="0" class="form-control" placeholder="Enter item Price">
...
<input type="hidden" name="sale_estim_final_amount" value="{{ old('sale_estim_final_amount', $previewData['sale_estim_final_amount'] ?? 0) }}"> 
                ...
                <div class="col-md-4">
                    <div class="d-flex align-items-center">
                        ...
                        <input type="text" class="form-control" name="sale_estim_discount_desc"
                            aria-describedby="inputGroupPrepend" value="{{ old('sale_estim_discount_desc', $previewData['sale_estim_discount_desc'] ?? '') }}" placeholder="Description (optional)">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="d-flex">
                        <input type="number" class="form-control form-controltext" name="sale_estim_item_discount"
                            placeholder="Enter a discount value" min="1" aria-describedby="inputGroupPrepend" value="{{ old('sale_estim_item_discount', $previewData['sale_estim_item_discount'] ?? '') }}">
                        <select class="form-select form-selectcurrency" id="sale_estim_discount_type"
                            name="sale_estim_discount_type">
                                <option value="1" {{ old('sale_estim_discount_type', $previewData['sale_estim_discount_type'] ?? 1) == 1 ? 'selected' : '' }}>
                                    {{ $currency->currency_symbol ?? '' }}
                                </option>
                                <option value="2" {{ old('sale_estim_discount_type', $previewData['sale_estim_discount_type'] ?? 2) == 2 ? 'selected' : '' }}>
                                    %
                                </option>
                        </select>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table total_table">
                            <tr>
                                ...
                                <td id="sub-total">{{ $currency->currency_symbol }}{{ old('sale_estim_sub_total', $previewData['sale_estim_sub_total'] ?? '0.00') }}</td>
                                </tr>
                            <tr>
                                <td>Discount :</td>
                                <td id="discount">{{ $currency->currency_symbol }}{{ old('sale_estim_discount_total', $previewData['sale_estim_discount_total'] ?? '0.00') }}</td>
                                </tr>
                            <tr>
                                <td>Tax :</td>
                                <td id="tax">{{ $currency->currency_symbol }}{{ old('sale_estim_tax_amount', $previewData['sale_estim_tax_amount'] ?? '0.00') }}</td>
                                </tr>
                            <tr>
                                <select name="sale_currency_id" id="sale_currency_id"
                                    ...
                                    @foreach($currencys as $curr)
                                        ...
                                        <option value="{{ $curr->id }}" data-symbol="{{ $curr->currency_symbol }}" {{ $curr->id == old('sale_currency_id', $currency->id) ? 'selected' : '' }}>
                                            {{ $curr->currency }} ({{ $curr->currency_symbol }}) -
                                            {{ $curr->currency_name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <td id="total"><strong>{{ $currency->currency_symbol }}{{ old('sale_estim_final_amount', $previewData['sale_estim_final_amount'] ?? '0.00') }}</strong></td>
                                </tr>
                        </table>
                ...
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <textarea id="inputDescription" name="sale_estim_notes" class="form-control" rows="3"
                            placeholder="Enter notes or terms of service that are visible to your customer">{{ old('sale_estim_notes', $previewData['sale_estim_notes'] ?? '') }}</textarea>
                    </div>
                </div>
            </div>
            <!-- /.row -->
                ...
                <div class="row justify-content-between">
                    <div class="col-md-12">
                        <textarea id="inputDescription" name="sale_estim_footer_note" class="form-control" rows="3"
                            placeholder="Enter a footer for this invoice (e.g. tax information, thank you note)">{{ old('sale_estim_footer_note', $previewData['sale_estim_footer_note'] ?? '') }}</textarea>
                    </div>
                </div>
                <!-- /.row -->
            </div>
        ...
        <div class="row py-20">
            <div class="col-md-12 text-center">
                <!-- <a href="#" id="preview-link" class="add_btn_br">Preview</a> -->
                <!-- Preview Button -->
                <button type="button" value="true" id="preview-btn" class="add_btn_br">Preview</button>
                <button type="button"  id="save-btn1" value="false" class="add_btn">Save & Continue</button>
...
<div class="modal fade" id="editbusiness_companyaddress" tabindex="-1" role="dialog"
          ...
          <!--<x-input-error class="mt-2" :messages="$errors->get('bus_company_name')" />-->
           <x-input-label for="company-business" :value="__('Company/Business')" />
              ...
              <x-text-input type="text" class="form-control" id="bus_company_name" placeholder="Enter Business Name"
                            ...
                            :value="old('bus_company_name', $businessDetails->bus_company_name)" />
              <!-- Error message span -->
              <span id="companyNameError" class="text-danger mt-2" style="display:none;">Please enter business name.</span>
              <x-input-error class="mt-2" :messages="$errors->get('bus_company_name')" />
          </div>
        </div>
        </div>
  ...
  <div class="modal fade" id="editcolum" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form method="post" id="estimateForm" action="{{ route('reinvoicesmenus.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                @csrf
                @method('patch')
                <div class="modal-body">
                                            ...
                                            <div class="col-md-3">
                                                <div class="icheck-primary d-flex align-items-center">
                                                    ...
                                                    <input type="radio" 
                                                           ...
                                                           name="{{ $menu->mtitle }}" 
                                                           value="{{ $child->mname }}" 
                                                           {{ isset($sessionData[$menu->mtitle]) && $sessionData[$menu->mtitle] === $child->mname ? 'checked' : '' }}>
                                                    <label for="{{ $child->mname }}">{{ $child->mtitle }}</label>
                                                    <input type="text" 
                                                           class="form-control mar_15" 
                                                           placeholder="" 
                                                           name="{{ $menu->mtitle }}_other" 
                                                           value="{{ $sessionData[$menu->mtitle . '_other'] ?? '' }}">
                                                </div>
                                            </div>
                                        @else
                                            <div class="col-md-3">
                                                <div class="icheck-primary">
                                                    ...
                                                    <input type="radio" 
                                                           ...
                                                           name="{{ $menu->mtitle }}" 
                                                           value="{{ $child->mtitle }}" 
                                                           {{ isset($sessionData[$menu->mtitle]) && $sessionData[$menu->mtitle] === $child->mtitle ? 'checked' : '' }}>
                    ...
                    <div class="colum_box">
                        <div class="row align-items-center">
                            @foreach($HideMenus as $hmenu)
                                <div class="col-md-3">
                                    <div class="icheck-primary">
                                        <!-- Checkbox for hide items -->
                                        <input type="checkbox" 
                                               id="{{ $hmenu->mname }}" 
                                               name="{{ $hmenu->mname }}" 
                                               {{ isset($sessionData[str_replace(' ', '_', $hmenu->mname)]) && $sessionData[str_replace(' ', '_', $hmenu->mname)] === 'on' ? 'checked' : '' }}>
                            ...
                            @foreach($HideDescription as $dmenu)
                                <div class="col-md-12">
                                    <div class="icheck-primary">
                                        <!-- Checkbox for hide descriptions -->
                                        <input type="checkbox" 
                                               id="{{ $dmenu->mname }}" 
                                               name="{{ $dmenu->mname }}" 
                                               {{ isset($sessionData[str_replace(' ', '_', $dmenu->mname)]) && $sessionData[str_replace(' ', '_', $dmenu->mname)] === 'on' ? 'checked' : '' }}>
                    ...
                    <!-- Additional Hide Settings Section -->
                    <div class="row pad-3">
                        <div class="col-md-12">
                            @foreach($HideSettings as $smenu)
                                <div class="icheck-primary">
                                    <!-- Checkbox for hide settings -->
                                    <input type="checkbox" 
                                           id="{{ $smenu->mname }}" 
                                           name="{{ $smenu->mname }}" 
                                           {{ isset($sessionData[str_replace(' ', '_', $smenu->mname)]) && $sessionData[str_replace(' ', '_', $smenu->mname)] === 'on' ? 'checked' : '' }}>
  ...
  // Show customer dropdown list or customer info dynamically
$(document).ready(function () {
    // When "Choose a Different Customer" is clicked
    $('.customer_list').on('click', function () {
        var $customerList = $(this).next('.add_customer_list1');
        var $addCustomerBox = $(this);

        if ($customerList.is(':visible')) {
            $customerList.hide(); // Hide dropdown
            $addCustomerBox.show(); // Show "Choose a Different Customer"
        } else {
            $('#customerInfo').hide(); // Hide customer info
            $('.add_customer_list1').hide(); // Hide other dropdowns
            $('.customer_list').show(); // Reset "Choose a Different Customer" buttons
            $customerList.show(); // Show this dropdown
            $addCustomerBox.hide(); // Hide this button
        }
    });
    ...
    $('.add_customer_box').on('click', function () {
        ...
        if ($customerList.is(':visible')) {
            $customerList.hide(); // Hide dropdown
            $addCustomerBox.show(); // Show "Add Customer" box
        } else {
            $('.add_customer_list').hide(); // Hide other dropdowns
            $('.add_customer_box').show(); // Reset "Add Customer" buttons
            $customerList.show(); // Show this dropdown
            $addCustomerBox.hide(); // Hide this button
        }
    });
...
<script>
    ...
    function submitFormViaAjax(formData) {
      ...
      var method;
    <?php if (isset($previewData['reinvoices_id'])): ?>
        url = "{{ route('business.recurring_invoices.update', ['reinvoices_id' => $previewData['reinvoices_id']]) }}";
        method = 'PATCH';
    <?php else: ?>
  ...
  <script>
    ...
    $('#estimateForm').on('submit', function (e) {
      ...
      success: function (response) {
        ...
        updateTableHeaders();
                        ...
                        // Bootstrap 5 method to close modal
                        // Initialize Bootstrap 5 Modal
                    var myModal = new bootstrap.Modal(document.getElementById('editcolum'));
                    $('#editcolum').removeClass('show'); // Remove the 'show' class
                    $('.modal-backdrop').hide(); // Remove the backdrop if still present
                    $('body').removeClass('modal-open');
                    // Clear any inline styles applied (opacity-related styles)
                    $('#editcolum').css('display', 'none');

        // console.log($('#editcolum')); 
    ...
    function updateTableHeaders() {
      $.ajax({
      url: '{{ route('reinvoicesmenus.menulist') }}',
      type: 'GET',
      success: function (data) {
        console.log(data);
